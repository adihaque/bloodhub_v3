...
from bloodhub.models.models import (
    BloodRequest, DispatchWave, DonorOffer, DonorProfile, User, BloodCenter, AuditLog
)
...
def find_and_rank_candidates(
    ...
    for profile in profiles:
        ...
        # Respect donor's preferred maximum radius
        if dist_km > profile.preferred_radius_km:
            continue
...
def start_dispatch(db: DatabaseSession, request_id: str) -> Optional[DispatchWave]:
    """
    Initializes the wave dispatch engine for a blood request.
    """
    ...
    wave_config = settings.DEFAULT_WAVES[0]
    now = datetime.utcnow()
    ...
    if candidates:
        ...
        for cand in candidates:
            ...
            # Send urgent incoming alert
            notification_provider.send_dispatch_offer(
                cand["donor_id"],
                {
                    "offer_id": offer.id,
        ...
        # If no candidates in Wave 1, attempt immediate progression
        db.commit()
...
def progress_wave(db: DatabaseSession, request_id: str) -> Optional[DispatchWave]:
    ...
    if next_wave_num > len(settings.DEFAULT_WAVES):
        # All waves exhausted
        ...
        audit = AuditLog(
            ...
            details=f"All {len(settings.DEFAULT_WAVES)} waves exhausted without acceptance."
        )
        db.add(audit)
        db.add(request)
        db.commit()
    ...
    request.status = "MATCHING"
    wave_config = settings.DEFAULT_WAVES[next_wave_num - 1]
